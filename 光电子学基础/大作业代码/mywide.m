...
light1=normpdf(swavelength,swavelength(701),delwavelength*fix(length(wavelength)/6));%生成宽谱待测单峰光1
light2=normpdf(swavelength,swavelength(2501),delwavelength*fix(length(wavelength)/3));%生成宽谱待测单峰光2
light=light1/max(max(light1))+light2/max(max(light2))+0.05;%合成双峰光
%noise=0.02*(2*rand(size(light))-1).*light;%生成噪声
%light=light+noise;
...
save('widelight.mat','wavelength','current','responses');
